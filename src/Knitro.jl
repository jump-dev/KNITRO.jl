...
@docstrings
  ...
  solveProblem(kp::KnitroProblem) = solve_problem(kp, kp.x, kp.lambda, kp.status, kp.obj_val)
  solveProblem(kp::KnitroProblem,
               cons::Vector{Float64},
               objGrad::Vector{Float64},
               jac::Vector{Float64},
               hess::Vector{Float64},
               hessVector::Vector{Float64}) = solve_problem(kp, kp.x, kp.lambda, kp.status,
                                                            kp.obj_val, cons, objGrad, jac,
                                                            hess, hessVector)
