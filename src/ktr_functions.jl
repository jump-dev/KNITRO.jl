...
function set_linearvars(kp::KnitroProblem, linearVars::Vector{Cint})
    return_code = @ktr_ccall(set_linearvars, Int32, (Ptr{Void}, Ptr{Cint}),
                             kp.env, linearVars)
    # Returns 0 if OK, nonzero if error.
    if return_code != 0
...
function set_honorbnds(kp::KnitroProblem, honorBnds::Vector{Cint})
    return_code = @ktr_ccall(set_honorbnds, Int32, (Ptr{Void}, Ptr{Cint}),
                             kp.env, honorBnds)
    # Returns 0 if OK, nonzero if error.
    if return_code != 0
...
function addcompcons(kp::KnitroProblem,
                     ...
                     index1::Vector{Int32},
                     index2::Vector{Int32})
    return_code = @ktr_ccall(addcompcons, Int32, (Ptr{Void}, Cint, Ptr{Cint},
...
function chgvarbnds(kp::KnitroProblem,
                    x_L::Vector{Float64},
                    x_U::Vector{Float64})
    return_code = @ktr_ccall(chgvarbnds, Int32, (Ptr{Void},Ptr{Cdouble},
