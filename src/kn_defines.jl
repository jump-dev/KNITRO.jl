...
if KNITRO_VERSION >= v"13.0"
    const KN_LINSOLVER_PHASE_INIT       = Cint(0)
    const KN_LINSOLVER_PHASE_ANALYZE    = Cint(1)
    const KN_LINSOLVER_PHASE_FACTOR     = Cint(2)
    const KN_LINSOLVER_PHASE_SOLVE      = Cint(3)
    const KN_LINSOLVER_PHASE_FREE       = Cint(4)
    # multistart
    const KN_PARAM_MS_ENABLE            = Cint(1033)
    const KN_MS_ENABLE_NO               = Cint(0)
    const KN_MS_ENABLE_YES              = Cint(1)
    const KN_PARAM_MS_MAXSOLVES         = Cint(1034)
    const KN_PARAM_MS_MAXBNDRANGE       = Cint(1035)
    const KN_PARAM_MS_MAXTIMECPU        = Cint(1036)
    const KN_PARAM_MS_MAXTIMEREAL       = Cint(1037)
    const KN_PARAM_MS_NUMTOSAVE         = Cint(1051)
    const KN_PARAM_MS_SAVETOL           = Cint(1052)
    const KN_PARAM_MS_TERMINATE         = Cint(1054)
    const KN_MS_TERMINATE_MAXSOLVES     = Cint(0)
    const KN_MS_TERMINATE_OPTIMAL       = Cint(1)
    const KN_MS_TERMINATE_FEASIBLE      = Cint(2)
    const KN_MS_TERMINATE_ANY           = Cint(3)
    const KN_PARAM_MS_STARTPTRANGE      = Cint(1055)
    const KN_PARAM_MS_SEED              = Cint(1066)  
    const KN_PARAM_MS_DETERMINISTIC     = Cint(1078)
    const KN_MS_DETERMINISTIC_NO        = Cint(0)
    const KN_MS_DETERMINISTIC_YES       = Cint(1)
    # blis
    const KN_BLASOPTION_BLIS            = Cint(3)
    # other
    const KN_BAR_CONIC_ENABLE_AUTO      = Cint(-1)
    const KN_BAR_LINSYS_ELIMINATE_SLACKS= Cint(1)
    const KN_BAR_LINSYS_ELIMINATE_BOUNDS= Cint(2)
    const KN_BAR_LINSYS_ELIMINATE_INEQS = Cint(3)
    const KN_PARAM_BAR_LINSYS_STORAGE   = Cint(1129) 
    const KN_BAR_LINSYS_STORAGE_AUTO    = Cint(-1)
    const KN_BAR_LINSYS_STORAGE_LOWMEM  = Cint(1)
    const KN_BAR_LINSYS_STORAGE_NORMAL  = Cint(2)
    ...
    const KN_PARAM_BLAS_NUMTHREADS      = Cint(1135)
    const KN_PARAM_LINSOLVER_NUMTHREADS = Cint(1136)
    const KN_PARAM_MS_NUMTHREADS        = Cint(1137)
    const KN_PARAM_CONIC_NUMTHREADS     = Cint(1138)
    const KN_PARAM_NCVX_QCQP_INIT       = Cint(1139)
    const KN_NCVX_QCQP_INIT_AUTO        = Cint(-1)
    const KN_NCVX_QCQP_INIT_NONE        = Cint(0)
    const KN_NCVX_QCQP_INIT_LINEAR      = Cint(1)
    const KN_NCVX_QCQP_INIT_HYBRID      = Cint(2)
    const KN_NCVX_QCQP_INIT_PENALTY     = Cint(3)
    const KN_NCVX_QCQP_INIT_CVXQUAD     = Cint(4)
    const KN_PARAM_MIP_OPTGAPABS        = Cint(2004)
    const KN_PARAM_MIP_OPTGAPREL        = Cint(2005)
    const KN_PARAM_MIP_IMPLICATIONS     = Cint(2014)
    const KN_MIP_IMPLICATIONS_NO        = Cint(0)
    const KN_MIP_IMPLICATIONS_YES       = Cint(1)
    const KN_MIP_HEURISTIC_DIVING       = Cint(4)
    const KN_PARAM_MIP_CUTOFF           = Cint(2044)
    const KN_PARAM_MIP_HEUR_LNS         = Cint(2045)
    const KN_PARAM_MIP_MULTISTART       = Cint(2046)
    const KN_MIP_MULTISTART_OFF         = Cint(0)
    const KN_MIP_MULTISTART_ON          = Cint(1)
end

# -- THE CONSTANTS BELOW ARE DEPRECATED AND MOVED HERE 
const KN_PARAM_PAR_NUMTHREADS           = Cint(3001) # USE KN_PARAM_NUMTHREADS
const KN_PARAM_PAR_CONCURRENT_EVALS     = Cint(3002) # USE KN_PARAM_CONCURRENT_EVALS
const KN_PAR_CONCURRENT_EVALS_NO        = Cint(0)    # USE KN_CONCURRENT_EVALS_NO
const KN_PAR_CONCURRENT_EVALS_YES       = Cint(1)    # USE KN_CONCURRENT_EVALS_YES
const KN_PARAM_PAR_BLASNUMTHREADS       = Cint(3003) # USE KN_PARAM_BLAS_NUMTHREADS
const KN_PARAM_PAR_LSNUMTHREADS         = Cint(3004) # USE KN_PARAM_LINSOLVER_NUMTHREADS
const KN_PARAM_PAR_MSNUMTHREADS         = Cint(3005) # USE KN_PARAM_MS_NUMTHREADS
const KN_PAR_MSNUMTHREADS_AUTO          = Cint(0)    # DEPRECATED
# SOCP parameters
const KN_PARAM_PAR_CONICNUMTHREADS      = Cint(3006) # USE KN_PARAM_CONIC_NUMTHREADS